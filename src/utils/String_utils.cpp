...
namespace StringUtils {
    ...
    std::vector<uint8_t> base64_decode(const std::string& encoded_string) {
        ...
        if (in_len % 4 != 0) throw std::runtime_error("Base64 string length must be a multiple of 4.");
        ...
        while (in_len-- && (encoded_string[in_idx] != '=') && is_base64(encoded_string[in_idx])) {
            ...
            if (i == 4) { 
                for (i = 0; i < 4; i++) {
                    ...
                    if (pos == std::string::npos) throw std::runtime_error("Invalid character in Base64 string.");
        ...
        if (i) { // Handle padding characters ('=')
            for (j = i; j < 4; j++)
                char_array_4[j] = 0;
            ...
            for (j = 0; j < 4; j++) {
                if (j < i) { 
                    size_t pos = base64_chars.find(char_array_4[j]);
                     if (pos == std::string::npos) throw std::runtime_error("Invalid character in Base64 string during padding.");
            ...
            if (i > 1) ret.push_back(char_array_3[0]); // If at least two chars (e.g., "XY=="), first byte is valid.
            if (i > 2) ret.push_back(char_array_3[1]); // If at least three chars (e.g., "XYZ="), second byte is valid.
        }
        return ret;
    }
