...
std::vector<uint8_t> AES::gcm_encrypt(const std::vector<uint8_t>& plaintext, const std::vector<uint8_t>& iv, const std::vector<uint8_t>& aad) {
    ...
    try {
        if (!(ctx = EVP_CIPHER_CTX_new())) throw std::runtime_error("Failed to create EVP_CIPHER_CTX");
        ...
        ciphertext_buf.resize(plaintext.size() + EVP_MAX_BLOCK_LENGTH); // Max possible size
